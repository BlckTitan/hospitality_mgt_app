...
1. Operational Performance (Revenue & Core Activity)
These metrics assess how effectively the core hospitality assets (rooms, seats, services) are being utilized to generate revenue.

Metric,Calculation (Example for Hotels),Why it Matters
...
These ratios highlight cost control and how efficiently resources are being managed, which is critical in an industry with high fixed and variable costs.
...
These ratios assess the enterprise's ability to meet its short-term and long-term financial obligations.

Metric,Calculation,Why it Matters
Current Ratio,Current Assets/Current Liabilities,"A measure of short-term liquidity. A ratio between 1.2 and 2.0 is generally considered healthy, meaning the enterprise can cover its short-term debts."
Quick Ratio (Acid-Test Ratio),(Current Assets−Inventory)/Current Liabilities,"A stricter measure of liquidity, showing ability to pay short-term debt without relying on selling inventory (which can be difficult in a crunch)."
Debt-to-Equity Ratio,Total Debt/Shareholders’ Equity,"A measure of solvency and leverage, showing the proportion of debt used to finance assets relative to equity. A lower ratio indicates lower financial risk."
Interest Coverage Ratio,EBIT/Interest Expense,Measures the ability to cover interest payments on outstanding debt. A higher ratio indicates better health.
Cash Flow from Operations,(Found on the Cash Flow Statement),Measures the actual cash generated by the core business activities. Positive cash flow is paramount for survival.
...
Active bars / staff,Distinct bars and waiters in that period’s summaries,KPI row 1
Stock days finalized,Waiter–bar–day sessions where every `userStockLogs` row is `isFinalized`,KPI row 2
Open / stale reorders,Open count; unresolved (open + acknowledged) aged ≥ 24h; oldest alert age,KPI row 2 (`inventory.read`)
Revenue per waiter-shift,Live `salesValue` / distinct `shiftId` (or user+date),KPI row 2
Bar / waiter performance,Grouped `totalRevenue` (waiters also show per-shift),tabs
Revenue trend,Last N `periodKey`s; YoY two monthly series,tab
Sales by category / SKU,Revenue by `beverage.category`; top 5 and slowest 5 SKUs,tabs
