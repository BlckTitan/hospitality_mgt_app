...
export default function StoreInventoryPage() {
  ...
  // Fetch store inventories for the current property
  const inventoryData = useQuery(api.storeInventories.getAllStoreInventories, propertyId ? { propertyId: currentPropertyId } : "skip");
  ...
  if (inventoryData?.data?.length === 0) {
    return (
      <div className='w-full h-full flex justify-center items-center'>
        <p className='text-xl'>No inventory yet!</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full p-4 bg-white">
      ...
      <StoreInventory currentPropertyId={currentPropertyId} inventoryData={inventoryData}/>
