...
export function EditRoomForm({ roomData, roomId, onSuccess, onClose }: EditRoomFormProps) {
  const updateRoom = useMutation(api.rooms.updateRoom);

  const roomTypesResponse = useQuery(api.roomTypes.getAllRoomTypes, {
    propertyId: roomData.propertyId as Id<'properties'>,
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="editRoomForm">
      ...
      <div className="w-full h-fit flex flex-col lg:flex-row lg:justify-between lg:items-start gap-2 mb-2 lg:mb-4">
        <div className="flex-1">
          ...
          <select
            ...
            defaultValue={roomData.roomTypeId}
          >
      ...
      <div className="w-full h-fit flex flex-col lg:flex-row lg:items-center gap-4 mb-4">
        <label className="flex !items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            {...register('isActive')}
            defaultChecked={roomData.isActive ?? true}
